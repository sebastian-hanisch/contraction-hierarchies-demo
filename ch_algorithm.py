"""Contraction Hierarchies: Vorberechnung (Knoten nach Wichtigkeit zusammenziehen, Abkürzungen mit Zeugensuche) und Abfrage (bidirektionale Suche nur aufwärts, Route auspacken).

Eigene Umsetzung auf dem CSR-Graphen aus ch_graph.py; networkx kommt nur in den Tests vor (Kreuzprobe). Einseitiges Dijkstra und bidirektionale Suche als Vergleich stehen in ch_bd.py (Stück 3 der Linie)."""
...
def reweight(g, fraction, factor, seed=0):
    """Neue Kosten: ein Anteil der Kanten (zufällig, gerichtet einzeln) wird mit `factor` multipliziert und auf ganze Zahlen gerundet - der 'Verkehr' hat sich geändert. Rückgabe: neuer Graph."""
    from ch_graph import Graph
    rng = np.random.default_rng([int(seed), 1313])
    w = g.weight.copy()
    hit = rng.random(len(w)) < fraction
    w[hit] = np.maximum(1.0, np.rint(w[hit] * factor))
